#!/usr/bin/env python3
"""
FAST Import of new countries (Belgium, Spain, Germany, Ireland)
Optimized for speed with batching and efficient database operations
"""
...
class FastCountryImporter:
    """Fast import of new countries with batching"""
    ...
    def import_new_countries_fast(self):
        """Import data from ONLY the new countries with optimized performance"""
        print("🚀 Starting FAST import of NEW countries only...")
        ...
        try:
            ...
            self.import_belgium_data_fast(db)
            self.import_spain_data_fast(db)
            self.import_germany_data_fast(db)
            self.import_ireland_data_fast(db)
    ...
    def _get_or_create_manager_fast(self, db: Session, manager_name: str) -> Manager:
    ...
    def _get_or_create_company_fast(self, db: Session, company_name: str, isin: str, country_name: str) -> Company:
    ...
    def import_belgium_data_fast(self, db: Session):
        """Import Belgium data from CSV files with batching"""
        print("\n🇧🇪 Importing Belgium data (FAST)...")
        ...
        try:
            ...
            for idx, row in df_current.iterrows():
                try:
                    ...
                    manager = self._get_or_create_manager_fast(db, manager_name)
                    company = self._get_or_create_company_fast(db, company_name, isin, 'Belgium')
    ...
    def import_spain_data_fast(self, db: Session):
        """Import Spain data from Excel file with batching"""
        print("\n🇪🇸 Importing Spain data (FAST)...")
                ...
                try:
                    ...
                    for idx, row in df.iterrows():
                        try:
                            ...
                            # Get or create manager and company
                            manager = self._get_or_create_manager_fast(db, manager_name)
                            company = self._get_or_create_company_fast(db, company_name, isin, 'Spain')
    ...
    def import_germany_data_fast(self, db: Session):
        """Import Germany data from CSV file with batching"""
        print("\n🇩🇪 Importing Germany data (FAST)...")
        ...
        try:
            ...
            for idx, row in df.iterrows():
                try:
                    ...
                    # Get or create manager and company
                    manager = self._get_or_create_manager_fast(db, manager_name)
                    company = self._get_or_create_company_fast(db, company_name, isin, 'Germany')
    ...
    def import_ireland_data_fast(self, db: Session):
        """Import Ireland data from Excel file with batching"""
        print("\n🇮🇪 Importing Ireland data (FAST)...")
                ...
                try:
                    ...
                    for idx, row in df.iterrows():
                        try:
                            ...
                            # Get or create manager and company
                            manager = self._get_or_create_manager_fast(db, manager_name)
                            company = self._get_or_create_company_fast(db, company_name, isin, 'Ireland')
...
def main():
    """Main function"""
    print("📊 ShortSelling.eu - FAST New Countries Import Tool")
    print("=" * 80)
