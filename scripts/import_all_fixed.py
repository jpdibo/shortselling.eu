#!/usr/bin/env python3
"""
FIXED Import script for all countries
Properly handles _get_or_create_manager and is_current flags
"""
...
class FixedImporter:
    """Fixed import with proper manager handling"""
    ...
    def import_all_countries(self):
        ...
        print("🚀 Starting FIXED import of all countries...")
        print("📋 Countries: UK, Italy, Netherlands, France, Belgium, Spain, Germany, Ireland")
        print("⚡ Using proper _get_or_create_manager function")
    ...
    def _get_or_create_manager_fixed(self, db: Session, manager_name: str) -> Manager:
        """FIXED: Get or create a manager with proper error handling"""
    ...
    def _get_or_create_company_fixed(self, db: Session, company_name: str, isin: str, country_name: str) -> Company:
        """FIXED: Get or create a company with proper error handling"""
    ...
    def import_uk_data(self, db: Session):
        ...
        filepath = self.base_path / "United Kingdom" / "UK_Short_Positions.xlsx"
                ...
                try:
                    ...
                    for idx, row in df.iterrows():
                        try:
                            ...
                            manager = self._get_or_create_manager_fixed(db, manager_name)
                            company = self._get_or_create_company_fixed(db, company_name, isin, 'United Kingdom')
    ...
    def import_italy_data(self, db: Session):
        ...
        filepath = self.base_path / "Italy" / "Italy_Short_Positions.xlsx"
        if not filepath.exists():
                ...
                try:
                    ...
                    for idx, row in df.iterrows():
                        try:
                            ...
                            # Get or create manager and company
                            manager = self._get_or_create_manager_fixed(db, manager_name)
                            company = self._get_or_create_company_fixed(db, company_name, isin, 'Italy')
    ...
    def import_netherlands_data(self, db: Session):
        ...
        current_file = self.base_path / "Netherlands" / "Netherlands_Current.csv"
        history_file = self.base_path / "Netherlands" / "Netherlands_Historical.csv"
        ...
        try:
            ...
            for idx, row in df_current.iterrows():
                try:
                    ...
                    # Get or create manager and company
                    manager = self._get_or_create_manager_fixed(db, manager_name)
                    company = self._get_or_create_company_fixed(db, company_name, isin, 'Netherlands')
    ...
    def import_france_data(self, db: Session):
        ...
        filepath = self.base_path / "France" / "France_Short_Positions.csv"
        if not filepath.exists():
        ...
        try:
            ...
            for idx, row in df.iterrows():
                try:
                    ...
                    # Get or create manager and company
                    manager = self._get_or_create_manager_fixed(db, manager_name)
                    company = self._get_or_create_company_fixed(db, company_name, isin, 'France')
        ...
        try:
            ...
            for idx, row in df_current.iterrows():
                try:
                    ...
                    # Get or create manager and company
                    manager = self._get_or_create_manager_fixed(db, manager_name)
                    company = self._get_or_create_company_fixed(db, company_name, isin, 'Belgium')
                ...
                try:
                    ...
                    for idx, row in df.iterrows():
                        try:
                            ...
                            # Get or create manager and company
                            manager = self._get_or_create_manager_fixed(db, manager_name)
                            company = self._get_or_create_company_fixed(db, company_name, isin, 'Spain')
        ...
        try:
            ...
            for idx, row in df.iterrows():
                try:
                    ...
                    # Get or create manager and company
                    manager = self._get_or_create_manager_fixed(db, manager_name)
                    company = self._get_or_create_company_fixed(db, company_name, isin, 'Germany')
                ...
                try:
                    ...
                    for idx, row in df.iterrows():
                        try:
                            ...
                            # Get or create manager and company
                            manager = self._get_or_create_manager_fixed(db, manager_name)
                            company = self._get_or_create_company_fixed(db, company_name, isin, 'Ireland')
...
def main():
    """Main function"""
    print("📊 ShortSelling.eu - FIXED Import Tool")
    print("=" * 80)
    print("🚀 Importing ALL countries with FIXED manager handling")
    print("⚡ Proper _get_or_create_manager function")
